...
public class EditScheduleController {
    ...
    private void populateFields() {
        if (currentSchedule != null) {
            ...
            // Set day
            dayCombo.setValue(currentSchedule.getDayOfWeek());

            // Set slot
            String slotDisplay = currentSchedule.getClassSlot().getStartTime() + " - " + currentSchedule.getClassSlot().getEndTime();
            slotCombo.setValue(slotDisplay);

            // Set course
            courseCombo.setValue(currentSchedule.getCourse().getCourseTitle());

            // Set teacher
            teacherCombo.setValue(currentSchedule.getTeacher().getTeacherName());

            // Set batch
            batchCombo.setValue(currentSchedule.getBatch().getBatchName());

            // Set room
            roomCombo.setValue(currentSchedule.getClassRoom().getRoomName());
        }
    }
    ...
    private void handleUpdateSchedule() {
        ...
        try {
            ...
            currentSchedule.setDayOfWeek(day);
            currentSchedule.setClassSlot(slot);
            currentSchedule.setCourse(course);
            currentSchedule.setTeacher(teacher);
            currentSchedule.setBatch(batch);
            currentSchedule.setClassRoom(room);

            // Try to update schedule (clash detection happens in DAO)
            try {
            ...
            e.printStackTrace();
            showMessage("Error updating schedule: " + e.getMessage(), "error");
        }
    }
    ...
    private void closeWindow() {
        try {
            // Try getting stage from any control (they're all in the same window)
            Stage stage = null;
