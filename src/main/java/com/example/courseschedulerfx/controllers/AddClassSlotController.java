...
public class AddClassSlotController {
    ...
    private void handleSave() {
        ...
        try {
            ...
            if (!ClassSlotDAO.isTimeSlotAvailable(startTime, endTime, -1)) {
                showError("This time slot overlaps with an existing slot");
                return;
            }

            // Add the slot
            boolean success = ClassSlotDAO.addSlot(startTime, endTime);
            ...
            if (success) {
                ...
                showError("Failed to add time slot. Please try again.");
            }
        } catch (Exception e) {
            e.printStackTrace();
